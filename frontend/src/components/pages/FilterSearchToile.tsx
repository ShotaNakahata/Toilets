...
const FilterSearchToile: React.FC = () => {
    const [toiletList, setToiletList] = useState<Toilet[]>([]);
    const [originalToiletList, setOriginalToiletList] = useState<Toilet[]>([]); // 取得したデータを保存
    ...
    const toggleRatingFilter = () => {
        ...
        // フィルターが解除された場合は、全てのデータを再表示
        if (!newTopRated && !showUniversal) {
            setToiletList(originalToiletList);
        }
    };

    // Accessible フィルターのトグル
    const toggleUniversalFilter = () => {
        const newShowUniversal = !showUniversal;
        setShowUniversal(newShowUniversal);
        applyFilters(ref.current?.value || '', isTopRated, newShowUniversal);
        ...
        // フィルターが解除された場合は、全てのデータを再表示
        if (!newShowUniversal && !isTopRated) {
            setToiletList(originalToiletList);
        }
    };
        ...
        <div className="bg-background min-h-screen pt-20">
            <div className="container-SearchToile mx-auto p-6">
                <div className="SearchToile-wrapper bg-background p-6 rounded-lg shadow-md">
                    <h1 className="text-6xl font-bold text-white mb-5">Restroom list</h1>
                    <h2 className="text-2xl font-semibold text-white mb-4">Filter Search Mode</h2>
                    <Link to="/" className="home-button text-white bg-background hover:bg-white hover:text-background rounded-lg border-white border-2 py-2 px-4 transition-colors">
                        Return to Home
                    </Link>
                    <button
                        onClick={toggleRatingFilter}
                        className={`FilterOptions-button text-white bg-background hover:bg-white hover:text-background rounded-lg border-white border-2 py-1 px-4 ml-4 transition-colors ${
                            isTopRated ? 'bg-green-500' : ''
                        }`}
                    >
                        {isTopRated ? "Disable Top Rated" : "Enable Top Rated"}
                    </button>
                    <button
                        onClick={toggleUniversalFilter}
                        className={`FilterOptions-button text-white bg-background hover:bg-white hover:text-background rounded-lg border-white border-2 py-1 px-4 ml-4 transition-colors ${
                            showUniversal ? 'bg-green-500' : ''
                        }`}
                    >
                        {showUniversal ? "Disable Accessible" : "Enable Accessible"}
                    </button>
                    <input
                        type="text"
                        ref={ref}
