...
//             setToilet((prevToilet: IToilet | null) => {
//                 if (!prevToilet) return null;
...
//                                     className="flex-1 border-2 h-full border-white bg-background text-white  rounded-full hover:bg-white hover:text-gray-800 transition-colors px-2 py-2 duration-300 text-sm sm:text-xs sm:px-4 sm:py-2"
...
import DeleteButton from '../common/button/DeleteButton';
import { IToilet } from '../../interfaces/Toilet_Interfaces';
import { Comment } from '../../interfaces/Comment_Interfaces'; // インポートを統一
        ...
        <div className='p-4 pt-20 sm:pt-52 bg-background min-h-screen '>
            <div className='container flex flex-col md:flex-row space-y-8 md:space-y-0 md:space-x-8'>
                ...
                <div className='flex flex-col space-y-8 w-full md:w-1/2 mt-7'>
                    ...
                    <div className="sm:flex sm:space-x-2 w-full grid space-y-2">
                        <button
                            onClick={() => navigate(-1)}
                            className="flex-1 border-2 border-white bg-background text-white rounded-full hover:bg-white hover:text-gray-800 transition-colors px-2 py-2 duration-300 text-sm sm:text-xs sm:px-4 sm:py-2">
                            Go Back
                        ...
                        {/* 追加: 地図で表示ボタン */}
                        <button
                            onClick={() => navigate(`/map?toiletId=${toilet._id}`)}
                            className="flex-1 border-2 border-blue-500 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors px-2 py-2 duration-300 text-sm sm:text-xs sm:px-4 sm:py-2">
                            View on Map
                        </button>
                    </div>
                ...
                <div className='flex flex-col space-y-8 w-full md:w-1/2'>
                    <div className='text-3xl text-white leading-relaxed'>
                        <h2 className="text-3xl font-bold mb-4">Add Rating</h2>
                        <div className="p-4 border border-white rounded-lg shadow-white mb-10">
                            <RatingForm onSubmit={handleCommentSubmit} />
                        </div>
                        <h2 className="text-4xl font-bold mt-8 mb-4">Comments</h2>
                        <CommentsList comments={comments} />
